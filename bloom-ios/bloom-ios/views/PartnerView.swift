...
struct PartnerView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 24) {
                // Header
                header("You matter too", icon: "hands.sparkles.fill", color: Color(red: 0.22, green: 0.72, blue: 0.96))
                ...
                // Response
                if let response = service.response, response.pillar == .partner {
    ...
    private func contextSnapshot() -> some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            Divider().background(Color(.sRGB, red: 0.18, green: 0.18, blue: 0.24))
                .padding(.vertical, 10)

            contextRow(
                icon: "moon.stars",
                iconColor: Color(red: 0.4, green: 0.7, blue: 0.9),
                label: profile.babyData.babyName.isEmpty ? "Baby" : profile.babyData.babyName,
    ...
    private func contextRow(icon: String, iconColor: Color, label: String, value: String) -> some View {
        HStack(spacing: 12) {
        ...
        Button {
            ...
            .background(
                RoundedRectangle(cornerRadius: 16)
                    ...
                    .stroke(Color(red: 0.22, green: 0.72, blue: 0.96, opacity: 0.25), lineWidth: 1)
            )
        }
    }
            ...
            ForEach(questions, id: \.self) { q in
                Button {
                    ...
                    HStack {
                        Text(q)
                            .font(.system(size: 14))
                            .foregroundColor(Color(.sRGB, red: 0.72, green: 0.72, blue: 0.75))
                        Spacer()
    ...
    private var lastMoodLabel: String {
        guard let last = profile.moodHistory.last else { return "No check-in yet" }
